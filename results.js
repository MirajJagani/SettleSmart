...
function getSelectedUniversityName() {
  return preferences.university || "your selected university";
}

function getUniversityAccessBonus(suburb) {
  if (!preferences.university) return 0;

  if (suburb.university === "high") return 12;
  if (suburb.university === "medium") return 7;
  if (suburb.university === "low") return 2;

  return 0;
}

function getUniversityAccessReason(suburb) {
  if (!preferences.university) return "";

  if (suburb.university === "high") {
    return `strong access to ${getSelectedUniversityName()}`;
  }

  if (suburb.university === "medium") {
    return `reasonable access to ${getSelectedUniversityName()}`;
  }

  return `some access to ${getSelectedUniversityName()}`;
}

function updateReviewMoreDetailsCopy() {
  if (viewMoreBtn) {
    viewMoreBtn.innerHTML = `Review more details <span>→</span>`;
  }

  const shortlistDescription = document.querySelector("#detailView .section-header h6");

  if (shortlistDescription) {
    shortlistDescription.textContent =
      "Suburbs are ranked using your cultural preferences, language comfort, budget, housing style, commute preference, lifestyle priority, and selected university access.";
  }
}
...
function rankSuburbs() {
  return window.suburbs
    .filter((suburb) => suburb.city === preferences.city)
    .map((suburb) => {
      const baseScore = window.getSuburbScore(suburb, preferences);
      const universityBonus = getUniversityAccessBonus(suburb);
      const reasons = window.buildReasonList(suburb, preferences);
      const universityReason = getUniversityAccessReason(suburb);

      if (universityReason) {
        reasons.push(universityReason);
      }

      return {
        ...suburb,
        score: Math.min(100, baseScore + universityBonus),
        reasons
      };
    })
    .sort((a, b) => b.score - a.score);
  ...
  topMatchPanel.innerHTML = `
    <div class="top-match-inline-layout">
      <div class="top-match-main">
        ...
        <p class="top-match-copy">
          ${match.suburb} is recommended as your best match because it balances your cultural comfort,
          language support, and access to ${getSelectedUniversityName()}.
          Recommended because ${match.reasons.join(", ")}.
        </p>
      ...
      <div class="top-match-aside">
        ...
        <div class="top-match-focus-card">
          ...
          <div class="top-match-focus-pills">
            ...
            <span class="signal-pill">Lifestyle: ${window.formatChoice(preferences.lifestyle)}</span>
            <span class="signal-pill">Community: ${window.formatChoice(match.culture)}</span>
            <span class="signal-pill">University: ${window.formatChoice(match.university)}</span>
          </div>
        </div>
      </div>
    </div>
